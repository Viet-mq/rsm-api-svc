...
@Component
public class ReportRejectProfileExporter extends BaseExporter {
    public ExportResponse exportReportRejectProfile(List<ReportRejectProfileEntity> report, String excelFilePath, Long from, Long to) {
        ExportResponse response = new ExportResponse();
        try {
            // Create Workbook
            Workbook workbook = getWorkbook(excelFilePath);

            for (ReportRejectProfileEntity reportRejectProfileEntity : report) {
                // Create sheet
                Sheet sheet = workbook.createSheet(reportRejectProfileEntity.getSheet()); // Create sheet with sheet name
                ...
                // Write data
                int rowIndex = 5;
                for (Reason reason : reportRejectProfileEntity.getReasons()) {
                    // Create row
                    Row row = sheet.createRow(rowIndex);
                    // Write data on row
                    writeReport(reason, sheet, row, rowIndex - 4);
                    rowIndex++;
                }

                writeFooter(sheet, 5 + reportRejectProfileEntity.getReasons().size(), reportRejectProfileEntity.getTotal());
            }
    ...
    private static void writeFooter(Sheet sheet, int rowIndex, long sum) {
        // create CellStyle
        CellStyle cellStyle = createStyleForFooter(sheet);

        CellStyle cellStyleSum = createStyleForFooter(sheet);
        cellStyleSum.setAlignment(HorizontalAlignment.CENTER);

        CellStyle cellStylePercent = createStyleForFooter(sheet);
        ...
        CellRangeAddress ca = new CellRangeAddress(rowIndex, rowIndex, 0, 1);
        sheet.addMergedRegion(ca);
    }


    // Write data
    private static void writeReport(Reason report, Sheet sheet, Row row, int stt) {
        ...
        cell = row.createCell(1);
        cell.setCellStyle(cellStyle);
        cell.setCellValue(report.getReason());
    ...
    private static void writeBanner(Long from, Long to, Sheet sheet) {
        ...
        cell.setCellStyle(cellStyle);
        cell.setCellValue("BÁO CÁO THỐNG KÊ ỨNG VIÊN BỊ LOẠI TẠI VÒNG TUYỂN DỤNG ỨNG TUYỂN");
    ...
    private static CellStyle createStyleForBanner(Sheet sheet) {
        ...
        CellRangeAddress ca = new CellRangeAddress(1, 1, 0, 3);
        sheet.addMergedRegion(ca);
    ...
    private static CellStyle createStyleForBanner2(Sheet sheet) {
        ...
        CellRangeAddress ca = new CellRangeAddress(2, 2, 0, 3);
        sheet.addMergedRegion(ca);
